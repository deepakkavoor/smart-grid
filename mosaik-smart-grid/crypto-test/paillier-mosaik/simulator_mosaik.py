    ...
    def step(self, time, inputs):
        ...
        for eid, attrs in inputs.items():
            for attr, values in attrs.items():
                ...
                rec_data_list = [value for value in values.values()]
                new_rec_data = rec_data_list[0] if len(rec_data_list) == 1 else e_add(pubKey, rec_data_list[0], rec_data_list[1])
                for data_idx in range(2, len(rec_data_list)):
                    new_rec_data = e_add(pubKey, new_rec_data, rec_data_list[data_idx])
